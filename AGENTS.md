...
> Source of truth for scope/architecture/roadmap is `PROJECT_PLAN.md` at repo root. Read it first if you need context beyond "how do I run this." Individual architectural decisions and their reasoning live in `/docs/adr/`.
...
Four apps: `store` (public storefront), `admin` (internal dashboard), `api` (backend, Cloudflare Workers), `studio` (AI generation pipeline, Node, admin/editor-only). A fifth, `docs`, comes later (see `PROJECT_PLAN.md` §Roadmap Phase 7).
...
> These commands reflect the intended Turborepo setup per `PROJECT_PLAN.md` §3 (Development Approach) and §4 (Tech Stack). Update this section once the monorepo is actually scaffolded if any script names differ.
...
apps/
  ...
  docs/         # (later) Tanstack Start — renders PROJECT_PLAN.md, ADRs, OpenAPI specs
packages/
...
PROJECT_PLAN.md # source of truth for scope/architecture/roadmap
```

**Hard boundaries — do not cross these without first checking `PROJECT_PLAN.md` / relevant ADR:**

- `packages/db` (Drizzle schema, Postgres access) is imported by `api` only. `studio` does **not** get a Postgres connection — it uploads generated files directly to R2 and calls `api`'s endpoints to persist product metadata. See ADR-0001 and `PROJECT_PLAN.md` §6.3.
...
- **Trunk-based development**: `main` stays deployable. Short-lived branches, one PR per task, squash-merge. See `PROJECT_PLAN.md` §3 for the full rationale (this is a solo project — the PR is a review checkpoint, not a team ceremony).
- **Definition of Done = the "Exit criteria" line** at the bottom of each roadmap phase in `PROJECT_PLAN.md`. If you're not sure whether a phase is finished, check there before moving on.
...
- Anything that would expose `studio` or admin-only `api` routes publicly (e.g. while building the `docs` app's API reference) — Studio is explicitly admin/editor-only for now; public self-serve access is a deferred future feature (`PROJECT_PLAN.md` §13), not something to build toward by default.
